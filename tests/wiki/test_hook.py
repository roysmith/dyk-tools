...
def test_render(site, testcase):
    hook = Hook("", testcase.input)
    assert hook.render(site) == testcase.output


def test_render_logs_on_unknown_node(mocker, site, caplog):
...
def test_render_does_not_log_with_no_unknown_node(mocker, site, caplog, Page):
